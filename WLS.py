        ...
        while (distance > 1):
            ...
            for ii in range(0, N):
                ...
                for uu in range(0, len(np.array(list(itertools.combinations(kk, 2))))):
                    ...
                    b2.append(d_i[hh - 1] ** 2 - d_i[gg - 1] ** 2 - np.linalg.norm(a_i[0:3, hh - 1]) ** 2 + np.linalg.norm(
                        a_i[0:3, gg - 1]) ** 2)
                A2 = np.asarray(A2, dtype=np.float32)
                b2 = np.asarray(b2, dtype=np.float32)
                xi_est.append(np.linalg.solve(dot(A2.T, A2) + (1 * 10 ** (-6)) * np.eye(3), dot(A2.T, b2)))
                di_xy = np.linalg.norm(xi_est[0][0:2])
                xi_est[ii][2] = cmath.sqrt((d_i[0] ** 2) - (di_xy ** 2)).real + cmath.sqrt((d_i[0] ** 2) - (di_xy ** 2)).imag
