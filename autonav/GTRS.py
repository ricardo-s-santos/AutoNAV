        ...
        while distance > 1:
            ...
            if qq != 0:
                ...
                A1_track = np.concatenate(
                    (A1_track,
                     np.concatenate((sp.sqrtm(np.linalg.inv(P_pred)), np.zeros(np.size(x_state, 1), 1)), axis=1))
                    , axis=0
                )
                ...
                W_track = np.concatenate((np.dot(math.sqrt(1. / 2.), w_i_loc.T),
                                          np.dot(math.sqrt(1. / 8.), np.ones(np.size(x_state, 1)))), axis=0)
                ...
                D_track[1, 1] = 1
                D_track[2, 2] = 1
                f_track = np.array([0, 0, 0, 0, 0, 0, - 1. / 2.])
            ...
            if qq == 0:
                ...
                y_hat_track = np.linalg.solve(
                    (np.dot(A_track.T, A_track) + np.dot(lambda_track, D_track) + np.dot(1e-06,
                                                                                         np.eye(np.size(A_track, 2)))),
                    (np.dot(A_track.T, b_track) - np.dot(lambda_track, f_track)))

                x_state = np.concatenate((x_state, np.zeros(np.size(x_state, 1), 1)), axis=1)
                x_state[0:6, qq] = np.concatenate((np.real(y_hat_track[range(0, np.size(x_state, 1) - 1)])), axis=0)
